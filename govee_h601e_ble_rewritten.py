...
# Current known-good session bootstrap from live logs
...
# From fresh log with 30 s pauses
...
class GoveeH601E:
    ...
    async def connect_and_handshake(self, client: BleakClient) -> bool:
        ...
        log.info("Sende HS1 (20 Byte) mit write-without-response")
        ...
        if len(hs1_resp) >= 16:
            try:
                log.info("Live HS1 dec(first16): %s", hex_bytes(aes_ecb_decrypt(ROOT_KEY, hs1_resp[:16])))
            except Exception:
                pass

        await self._drain_queue()
        log.info("Sende HS2 (20 Byte) mit write-without-response")
    ...
    async def send_raw(self, client: BleakClient, packet: bytes, label: str, wait_reply: float = 1.5) -> Optional[bytes]:
        await self._drain_queue()
        log.info("Sende %s (%d Byte) mit write-without-response", label, len(packet))
        ...
        resp = await self._wait_for_new_notification(timeout=wait_reply, ignore=self._seen_packets)
        if resp is not None:
...
async def run(args: argparse.Namespace) -> int:
    ...
    async with BleakClient(args.mac, timeout=15.0) as client:
        ...
        if args.test:
            ...
            await ctl.send_raw(client, packet, label)
            did_anything = True

        for idx, raw_hex in enumerate(args.raw_cmd_hex or [], start=1):
            pkt = bytes.fromhex(raw_hex)
            await ctl.send_raw(client, pkt, f"RAW-CMD-{idx}")
            did_anything = True
...
def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Govee H601E BLE test script")
    ...
    p.add_argument("--action-delay", type=float, default=5.0, help="Sekunden nach Handshake bis zum ersten Testkommando")
    p.add_argument("--idle-after", type=float, default=6.0, help="Sekunden nach dem letzten Testkommando offen bleiben")
    return p
